...
Queremos invitar a la mayor cantidad de invitados que vendrian segun una lista de empresas posibles, pero existen conflictos entre algunas de ellas,
por lo que no todas pueden estar juntas en el evento.

Empresas involucradas: e1, e2, e3, e4, e5, e6
Donde cada empresa proporcionaría:
- e1: 100 invitados
- e2: 150 invitados
- e3: 120 invitados
- e4: 80 invitados
...
# --------------------------------------------------
# Entiendo que el cambio sería aqui unicamente, en vez de dar el valor de 1, daran el valor de la lista de values
values = [100,150,120,80,80,200]

for i in range(1, 7):
    qubo[(f"e{i}",)] = values[i-1]
...
qubo[(f"e{3}", f"e{4}")] = -1*max(values)

# Si invitas a e1, no puedes invitar a e2 ni e3 → penalizaciones
qubo[(f"e{1}", f"e{2}")] = -2*max(values)
qubo[(f"e{1}", f"e{3}")] = -1*max(values)

# Si invitas a e2, no puedes invitar a e6 → penalización
qubo[(f"e{2}", f"e{6}")] = -1*max(values)
